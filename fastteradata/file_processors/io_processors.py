...
def concat_files_horizontal(data_file, data_files, col_list, primary_keys, dtype_dict):
    ...
    for clist, d_file in zip(col_list,data_files):
        ...
        df = pd.DataFrame()
        try:
            df = pd.read_csv(d_file, names=clist, sep="|", dtype=dtype_dict, na_values=["?","","~","!","null"])
        except:
            pass
        if len(df) == 0:
            df = pd.read_csv(d_file, names=clist, sep="|", dtype=dtype_dict, na_values=["?","","~","!","null"], encoding='latin1')
        #print(df)
        if len(_df) == 0:
            _df = df
        else:
