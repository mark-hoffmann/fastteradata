...
valid_final_distinct = ".LOGTABLE database1.fexplog; \n\n.LOGON /username, password; \n\n.BEGIN EXPORT; \n\n .EXPORT OUTFILE /data/ \n MODE RECORD FORMAT TEXT;\n\nSELECT DISTINCT TOP 10 CAST(\nCOALESCE(CAST(col1 AS CHAR(31)),'')||'|'||\nCOALESCE(CAST(col2 AS CHAR(20)),'')\n AS CHAR(53))\nFROM database1.table1\n;\n.END EXPORT;\n\n .LOGOFF;"

def test_generate_sql_main_output_file_simple_col_list():
    final, col_list = generate_sql_main(export_path, file_name, env_short, usr, passw, db, table, meta_df,
...
def test_generate_sql_main_output_file_simple_distinct():
    final, col_list = generate_sql_main(export_path, file_name, env_short, usr, passw, db, table, meta_df,
                                            columns=columns, nrows=nrows, partition_key=partition_key,
                                            current_partition=current_partition, partition_type=partition_type, distinct=True)
    assert valid_final_distinct == final
...
def test_generate_sql_main_output_file_explicit_columns():
    final, col_list = generate_sql_main(export_path, file_name, env_short, usr, passw, db, table, meta_df_dates, columns=["col1"])

    assert valid_final_explicit_columns == final

def test_generate_sql_main_output_partition_key_mismatch_exception():
    with pytest.raises(Exception):
        generate_sql_main(export_path, file_name, env_short, usr, passw, db, table, meta_df_dates, columns=["col1"], partition_key="invalid")
...
def test_coalesce_statement_date_end():
    stm = coalesce_statement("col1","DATE FORMAT 'YYYY-MM-DD') AS CHAR(10)",end=True)
...
def test_coalesce_statement_date_noend():
    stm = coalesce_statement("col1","DATE FORMAT 'YYYY-MM-DD') AS CHAR(10)",end=False)
    assert valid_coalesce2 == stm

valid_coalesce3 = "COALESCE(CAST(col1 AS CHAR(50)),'')\n"
def test_coalesce_statement_nodate_end():
    stm = coalesce_statement("col1","CHAR(50)",end=True)
    assert valid_coalesce3 == stm

valid_coalesce4 = "COALESCE(CAST(col1 AS CHAR(50)),'')||'|'||\n"
def test_coalesce_statement_nodate_noend():
    stm = coalesce_statement("col1","CHAR(50)",end=False)
    assert valid_coalesce4 == stm

valid_coalesce5 = "COALESCE(CAST(CAST(col1 AS DECIMAL(12,2) FORMAT 'Z99999999999.99') AS CHAR(15)),'')\n"
def test_coalesce_statement_number_end():
    stm = coalesce_statement("col1","DECIMAL(12,2) FORMAT 'Z99999999999.99') AS CHAR(15)",end=True)
    assert valid_coalesce5 == stm

valid_coalesce6 = "COALESCE(CAST(CAST(col1 AS DECIMAL(12,2) FORMAT 'Z99999999999.99') AS CHAR(15)),'')||'|'||\n"
def test_coalesce_statement_number_noend():
    stm = coalesce_statement("col1","DECIMAL(12,2) FORMAT 'Z99999999999.99') AS CHAR(15)",end=False)
